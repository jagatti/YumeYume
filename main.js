const GAS_URL = 'https://script.google.com/macros/s/AKfycbxbCTegDMq3C9T64VxxdGh-FXIfqtnJ17vx0zabXuNn_IVZObA3-JIyJwT7HbRRLZ08/exec';
...
function updateACOnTap(pointsWithCombo, nowTime) {
  ...
  acList.forEach(ac => {
    if (ac.state === "active" && nowTime >= ac.startTime + noteTravelSec && nowTime <= ac.endTime + noteTravelSec) {
      if (ac.type === "score") { ac.tapScore += pointsWithCombo; ac.progress = ac.tapScore + ac.spScore; }
      if (!ac.cleared && ac.progress >= ac.target) {
        ac.cleared = true; ac.state = "cleared"; score += ac.rewardScore; spValue = Math.min(SP_MAX, spValue + ac.rewardSP);
        skillHistory.unshift({text: `ACクリア報酬 ${ac.rewardScore}`, life:180}); if(skillHistory.length>5) skillHistory.pop();
        if(seededRandom() < 0.3){ permanentScoreBuff++; skillHistory.unshift({text:"[アピール増加永続 5%]", life:180}); if(skillHistory.length>5) skillHistory.pop(); }
      }
    }
    if (ac.state === "active" && nowTime > ac.endTime + noteTravelSec && !ac.cleared) {
      ac.state = "ended"; acFailFlashTimer = 18;
      skillHistory.unshift({text: "AC失敗！", life:180}); if(skillHistory.length>5) skillHistory.pop();
    }
  });
}
...
function updateACOnSPUse(nowTime, spScore) {
  ...
  acList.forEach(ac => {
    if (ac.state === "active" && nowTime >= ac.startTime + noteTravelSec && nowTime <= ac.endTime + noteTravelSec) {
      if (ac.type === "sp") { ac.progress += 1; }
      if (ac.type === "score") { ac.spScore += spScore; ac.progress = ac.tapScore + ac.spScore; }
      if (!ac.cleared && ac.progress >= ac.target) {
        ac.cleared = true; ac.state = "cleared"; score += ac.rewardScore; spValue = Math.min(SP_MAX, spValue + ac.rewardSP);
        skillHistory.unshift({text: `ACクリア報酬 ${ac.rewardScore}`, life:180}); if(skillHistory.length>5) skillHistory.pop();
      }
...
function calcTapBase(){ let baseRaw = 30000; if (appealBoostNotes > 0) { baseRaw = Math.ceil(baseRaw * 1.12); appealBoostNotes--; } return Math.min(50000, baseRaw); }
function getSimultaneousPairsInNotes() { const pairs = []; const used = new Set(); for (let i = 0; i < notes.length; i++) { if (used.has(i)) continue; const ni = notes[i]; for (let j = i + 1; j < notes.length; j++) { if (used.has(j)) continue; const nj = notes[j]; if (ni.chartIdx !== undefined && nj.chartIdx !== undefined) { const ci = notesChart[ni.chartIdx]; const cj = notesChart[nj.chartIdx]; if (ci && cj && ci.time === cj.time && ci.side !== cj.side) { pairs.push([ni, nj]); used.add(i); used.add(j); break; } } } } return pairs; }

function calcTapScoreAndLabel(dist, baseRaw){
  let label='WONDERFUL', mult=1.2;
  if(dist<=10){label='WONDERFUL';mult=1.2;} else if(dist<=18){label='GREAT';mult=1.1;} else if(dist<=24){label='NICE';mult=1.0;} else if(dist<=28){label='BAD';mult=0.9;} else {return {points:0,label:'MISS',reset:true};}
  let points=Math.floor(baseRaw*mult); if(seededRandom()<0.3){ points=Math.floor(points*1.5); label='CRITICAL'; } if(spBoostTimer>0) points=Math.floor(points*1.1); points=Math.min(50000, points);
  const reset = (label==='NICE' || label==='BAD'); return {points,label,reset};
}
...
function awardHit(target, points, label, resetCombo, baseRaw, chartIdx){
  ...
  for(const ac of acList){ if(ac.state === "waiting" && chartIdx === ac.startIdx){ ac.state = "active"; if(seededRandom() < 0.3){ permanentScoreBuff++; skillHistory.unshift({text:"[アピール増加永続 5%]", life:180}); if(skillHistory.length>5) skillHistory.pop(); } } }
  updateACOnTap(pointsWithCombo, nowTime);
}

function applyMiss(label='MISS'){ if(spValue<SP_MAX) spValue=Math.max(0, spValue-300); combo=0; const midX = (leftTarget.x + rightTarget.x) / 2; const midY = (leftTarget.y + rightTarget.y) / 2 - R*2; addPopup(label, midX, midY - 30, 500, 'label'); if(judgeCount[label] !== undefined) judgeCount[label]++; }
...
function tryUseSP(mx,my){
  ...
  for(let i=0;i<6;i++){ if(seededRandom()<0.5){ spValue = Math.min(SP_MAX, spValue + 600); skillHistory.unshift({text:`[SPゲージ獲得]`, life:180}); if(skillHistory.length>5) skillHistory.pop(); } }
  if(seededRandom() < 0.5){ spScoreBuffNotes = 15; skillHistory.unshift({text: "[アピール増加 10%]", life:180}); if(skillHistory.length>5) skillHistory.pop(); }
  updateACOnSPUse(nowTime, spScore); return true;
}

function handlePointer(e){
  if(gameState!=="playing") return;
  const isTouch = e.type.startsWith('touch'); let fingers = 1; if(isTouch){ lastInputWasTouch=true; e.preventDefault(); fingers = (e.touches && e.touches.length) ? e.touches.length : 1; } if(!isTouch && lastInputWasTouch){ lastInputWasTouch=false; return; }
  if(isTouch){ for(const t of e.touches){ const rect = cvs.getBoundingClientRect(); const scaleX = cvs.width / rect.width; const scaleY = cvs.height / rect.height; const mx = (t.clientX-rect.left)*scaleX; const my = (t.clientY-rect.top)*scaleY; if(isInSPSemicircle(mx,my)){ if(spValue >= SP_MAX){ tryUseSP(mx,my); } return; } } }
  else{ const rect = cvs.getBoundingClientRect(); const scaleX = cvs.width / rect.width; const scaleY = cvs.height / rect.height; const mx = (e.clientX-rect.left)*scaleX; const my = (e.clientY-rect.top)*scaleY; if(isInSPSemicircle(mx,my)){ if(spValue >= SP_MAX){ tryUseSP(mx,my); } return; } }
  if(fingers >= 2){ const pairs = getSimultaneousPairsInNotes(); for (const [nL, nR] of pairs) { let right = nL, left = nR; if(notesChart[nL.chartIdx]?.side === "left" && notesChart[nR.chartIdx]?.side === "right"){ left = nL; right = nR; } const posR = cubicBezier(right.path.p0, right.path.p1, right.path.p2, right.path.p3, Math.min(1, right.t/right.duration)); const distR = Math.hypot(posR.x - rightTarget.x, posR.y - rightTarget.y); const baseRaw = calcTapBase(); const resR = calcTapScoreAndLabel(distR, baseRaw); if(resR.label !== 'MISS'){ awardHit(rightTarget, resR.points, resR.label, resR.reset, baseRaw, right.chartIdx); notes = notes.filter(n => n !== right); } const posL = cubicBezier(left.path.p0, left.path.p1, left.path.p2, left.path.p3, Math.min(1, left.t/left.duration)); const distL = Math.hypot(posL.x - leftTarget.x, posL.y - leftTarget.y); const resL = calcTapScoreAndLabel(distL, baseRaw); if(resL.label !== 'MISS'){ awardHit(leftTarget, resL.points, resL.label, resL.reset, baseRaw, left.chartIdx); notes = notes.filter(n => n !== left); } } return; }
  function isNotPairNote(n){ return !notes.some(other => other !== n && notesChart[n.chartIdx]?.time === notesChart[other.chartIdx]?.time && notesChart[n.chartIdx]?.side !== notesChart[other.chartIdx]?.side ); }
  let targetNotes = notes.filter(isNotPairNote); let bestL = null, bestDistL = Infinity; for(const n of targetNotes){ if(n.side !== 'left') continue; const pos = cubicBezier(n.path.p0, n.path.p1, n.path.p2, n.path.p3, Math.min(1, n.t/n.duration)); const dist = Math.hypot(pos.x-leftTarget.x, pos.y-leftTarget.y); if(dist < bestDistL){ bestDistL = dist; bestL = n; } }
  if(bestL){ const baseRaw = calcTapBase(); const res = calcTapScoreAndLabel(bestDistL, baseRaw); if(res.label !== 'MISS'){ awardHit(leftTarget, res.points, res.label, res.reset, baseRaw, bestL.chartIdx); notes = notes.filter(n => n !== bestL); } }
  let bestR = null, bestDistR = Infinity; for(const n of targetNotes){ if(n.side !== 'right') continue; const pos = cubicBezier(n.path.p0, n.path.p1, n.path.p2, n.path.p3, Math.min(1, n.t/n.duration)); const dist = Math.hypot(pos.x-rightTarget.x, pos.y-rightTarget.y); if(dist < bestDistR){ bestDistR = dist; bestR = n; } }
  if(bestR){ const baseRaw = calcTapBase(); const res = calcTapScoreAndLabel(bestDistR, baseRaw); if(res.label !== 'MISS'){ awardHit(rightTarget, res.points, res.label, res.reset, baseRaw, bestR.chartIdx); notes = notes.filter(n => n !== bestR); } }
}
...
startBtn.onclick = function() { if(gameState === "init"){ startGame(Date.now()); } };
retryBtn.onclick = ()=>{ gameState = "init"; resizeCanvas(); try{ bgm.pause(); }catch(e){} bgm.currentTime = 0; permanentScoreBuff = 0; acList.forEach(ac=>{ ac.state = "waiting"; ac.progress = 0; ac.cleared = false; ac.tapScore = 0; ac.spScore = 0; }); };
reseedBtn.onclick = function() { startGame(lastGameSeed); };
...
function update(){
  ...
  if(spFlashTimer>0) spFlashTimer--; if(spRingTimer>0) spRingTimer--; if(spBoostTimer>0) spBoostTimer--;
...
function drawNotes(){
  ...
  for(let i=0;i<notes.length;i++){
    const n = notes[i]; const noteInfo = notesChart[n.chartIdx]; if(!noteInfo) continue; const idx = n.chartIdx; let isAcCleared = false; for(const ac of acList){ if(ac.state === "cleared" && ac.startIdx !== -1 && ac.endIdx !== -1 && idx >= ac.startIdx && idx <= ac.endIdx){ isAcCleared = true; break; } }
    let mainColor = isAcCleared ? "#ffd700" : "#00eaff"; let glowColor = isAcCleared ? "rgba(255,220,60,0.7)" : "rgba(0,200,255,0.7)"; let rimColor  = isAcCleared ? "#ffe777" : "#7fffff"; let dotColor  = isAcCleared ? "#ffe066" : "#1cd9ee";
    const pos = cubicBezier(n.path.p0, n.path.p1, n.path.p2, n.path.p3, Math.min(1, n.t/n.duration)); const r = R;
    ctx.save(); ctx.globalAlpha = 0.90; ctx.beginPath(); ctx.arc(pos.x, pos.y, r * 1.15, 0, Math.PI * 2); const glow = ctx.createRadialGradient(pos.x, pos.y, r * 0.6, pos.x, pos.y, r * 1.2); glow.addColorStop(0, glowColor); glow.addColorStop(0.7, isAcCleared ? "rgba(255,220,60,0.15)" : "rgba(0,200,255,0.18)"); glow.addColorStop(1, "rgba(0,0,0,0)"); ctx.fillStyle = glow; ctx.fill(); ctx.restore();
    ctx.save(); ctx.strokeStyle = rimColor; ctx.lineWidth = r * 0.18; ctx.globalAlpha = 0.82; ctx.beginPath(); ctx.arc(pos.x, pos.y, r * 0.87, 0, Math.PI * 2); ctx.stroke(); ctx.restore();
    ctx.save(); ctx.beginPath(); ctx.arc(pos.x, pos.y, r * 0.72, 0, Math.PI * 2); ctx.fillStyle = mainColor; ctx.shadowColor = mainColor; ctx.shadowBlur = r * 0.16; ctx.globalAlpha = isAcCleared ? 0.72 : 0.6; ctx.fill(); ctx.restore();
    ctx.save(); ctx.beginPath(); ctx.arc(pos.x, pos.y, r * 0.22, 0, Math.PI * 2); ctx.fillStyle = dotColor; ctx.globalAlpha = 0.88; ctx.fill(); ctx.restore();
    for(const ac of acList){ if(ac.startIdx === idx){ ctx.font = `bold ${Math.round(R*0.8)}px system-ui`; ctx.textAlign = "center"; ctx.lineWidth = 4; ctx.strokeStyle = "#fff"; ctx.strokeText("START", pos.x, pos.y+R+13); ctx.fillStyle = "#ff69b4"; ctx.fillText("START", pos.x, pos.y+R+13); } if(ac.endIdx === idx){ ctx.font = `bold ${Math.round(R*0.8)}px system-ui`; ctx.textAlign = "center"; ctx.lineWidth = 3; ctx.strokeStyle = "#ff69b4"; ctx.strokeText("FINISH", pos.x, pos.y+R+13); ctx.fillStyle = "#fff"; ctx.fillText("FINISH", pos.x, pos.y+R+13); } }
  }
}
...
function strokeRainbowText(text,x,y,font){ ctx.textAlign='center'; ctx.font=font; const g=ctx.createLinearGradient(x-100,y,x+100,y); const hue=(frame*4)%360; for(let i=0;i<=6;i++) g.addColorStop(i/6,`hsl(${(hue+i*60)%360},100%,50%)`); ctx.lineWidth=4; ctx.strokeStyle=g; ctx.strokeText(text,x,y); ctx.fillStyle='#fff'; ctx.fillText(text,x,y); }
function strokeColoredText(text,x,y,font,color){ ctx.textAlign='center'; ctx.font=font; ctx.lineWidth=4; ctx.strokeStyle=color; ctx.strokeText(text,x,y); ctx.fillStyle='#fff'; ctx.fillText(text,x,y); }
function strokeOrangeWhiteText(text,x,y,font){ ctx.textAlign='center'; ctx.font=font; ctx.lineWidth=6; ctx.strokeStyle='#fff'; ctx.strokeText(text,x,y); ctx.fillStyle='#ffa500'; ctx.fillText(text,x,y); }
function drawProgressBarWithAC(){
  const barMarginLeft = 20; const barMarginRight = 200; const barWidth = Math.max(140, cvs.width - barMarginLeft - barMarginRight); const barHeight = 12; const x = barMarginLeft; const y = 10;
  ctx.fillStyle = '#4b5563'; ctx.fillRect(x, y, barWidth, barHeight);
  let lastNoteTime = notesChart[notesChart.length-1]?.time || 0; let fallbackSongLen = lastNoteTime + noteTravelSec + 3; let songLen = (bgm.duration && !isNaN(bgm.duration) && bgm.duration > 1) ? bgm.duration : fallbackSongLen;
  for(const ac of acList){ let startRatio = (ac.startTime + noteTravelSec) / songLen; let endRatio = (ac.endTime + noteTravelSec) / songLen; let width = barWidth * (endRatio - startRatio); if(isNaN(startRatio) || isNaN(endRatio) || width <= 0) continue; ctx.fillStyle = "#ff69b4"; ctx.globalAlpha = 0.8; ctx.fillRect( x + barWidth * startRatio, y, width, barHeight ); ctx.globalAlpha = 1; }
  let progress = 0; if (bgm.duration && bgm.duration > 0) { progress = Math.min(1, bgm.currentTime / bgm.duration); }
  ctx.fillStyle = '#00e5ff'; ctx.fillRect(x, y, barWidth * progress, barHeight); ctx.strokeStyle = '#fff'; ctx.lineWidth = 2; ctx.strokeRect(x, y, barWidth, barHeight);
}
function drawSkillHistory(){ const maxItems = 5; const lineH = Math.max(16, Math.round(R*0.6)); const padding = 6; const x = 22; const startY = cvs.height - 22 - (maxItems-1)*lineH; ctx.textAlign='left'; ctx.font=`bold ${lineH}px system-ui`; const items = skillHistory.slice(0, maxItems); for(let i=0;i<items.length;i++){ const h=items[i]; let alpha = 1; if(h.life < 30) alpha = h.life/30; ctx.globalAlpha = alpha; const text = h.text; const w = ctx.measureText(text).width; ctx.fillStyle='rgba(0,0,0,0.4)'; ctx.fillRect(x-2, startY + i*lineH - (lineH-padding/2), w+16, lineH+padding/2); ctx.fillStyle='#e5faff'; ctx.fillText(text, x, startY + i*lineH); ctx.globalAlpha = 1; } }
function drawTargets(){ ctx.strokeStyle='#fff'; ctx.lineWidth=3; for(const t of [leftTarget,rightTarget]){ ctx.beginPath(); ctx.arc(t.x,t.y,t.r,0,Math.PI*2); ctx.stroke(); } }
function drawHitRings(){ for(const ring of hitRings){ ctx.strokeStyle=`rgba(255,255,255,${ring.alpha})`; ctx.lineWidth=3; ctx.beginPath(); ctx.arc(ring.x,ring.y,ring.r,0,Math.PI*2); ctx.stroke(); } }
function drawSPGauge(){ const cx=cvs.width/2, cy=cvs.height-10, radius=spRadius; const startAngle=Math.PI, endAngle=0; ctx.strokeStyle='#fff'; ctx.lineWidth=4; ctx.beginPath(); ctx.arc(cx,cy,radius,startAngle,endAngle,false); ctx.stroke(); const fillRatio=Math.min(1, spValue/SP_MAX); const fillEnd=startAngle + Math.PI*fillRatio; ctx.fillStyle='#fff'; ctx.beginPath(); ctx.moveTo(cx,cy); ctx.arc(cx,cy,radius,startAngle,fillEnd,false); ctx.closePath(); ctx.fill(); const nx=cx+Math.cos(fillEnd)*(radius-10), ny=cy+Math.sin(fillEnd)*(radius-10); ctx.strokeStyle='#fff'; ctx.lineWidth=3; ctx.beginPath(); ctx.moveTo(cx,cy); ctx.lineTo(nx,ny); ctx.stroke(); ctx.fillStyle='#fff'; ctx.beginPath(); ctx.arc(cx,cy,Math.max(3, radius*0.06),0,Math.PI*2); ctx.fill(); if(spValue >= SP_MAX){ ctx.save(); ctx.globalAlpha = 0.33; ctx.strokeStyle = '#ffff99'; ctx.lineWidth = 16; ctx.beginPath(); ctx.arc(cx, cy, radius+6, startAngle, endAngle, false); ctx.stroke(); ctx.globalAlpha = 1; ctx.restore(); } if(spFlashTimer>0){ ctx.fillStyle=`rgba(255,255,255,${spFlashTimer/10})`; ctx.beginPath(); ctx.moveTo(cx,cy); ctx.arc(cx,cy,radius,startAngle,endAngle,false); ctx.closePath(); ctx.fill(); } if(spRingTimer>0){ const rp=1 - spRingTimer/spRingSpeed; const rr=radius + rp*spRingRange; ctx.strokeStyle=`rgba(255,255,255,${1-rp})`; ctx.lineWidth=6; ctx.beginPath(); ctx.arc(cx,cy,rr,startAngle,endAngle,false); ctx.stroke(); } }
function drawPopups(){ for(const p of popups){ const a=p.timer/p.duration; ctx.globalAlpha=a; if(p.type==='label'){ let fontSize = Math.max(28, Math.round(R*1.2), Math.round(cvs.height*0.055)); if(p.text==='CRITICAL'){ ctx.fillStyle='#FFD700'; ctx.font=`bold ${fontSize}px system-ui`; ctx.textAlign='center'; ctx.fillText('★', p.x-fontSize*2, p.y); ctx.fillText('★', p.x+fontSize*2, p.y); strokeRainbowText('CRITICAL', p.x, p.y, `bold ${fontSize}px system-ui`); }else if(p.text==='WONDERFUL'){ strokeRainbowText('WONDERFUL', p.x, p.y, `bold ${fontSize}px system-ui`); }else if(p.text==='GREAT'){ strokeColoredText('GREAT', p.x, p.y, `bold ${fontSize}px system-ui`, '#ff69b4'); }else if(p.text==='NICE'){ strokeColoredText('NICE', p.x, p.y, `bold ${fontSize}px system-ui`, '#ffd700'); }else if(p.text==='BAD' || p.text==='MISS'){ strokeColoredText(p.text, p.x, p.y, `bold ${fontSize}px system-ui`, '#9ca3af'); } }else if(p.type==='score'){ let fontSize = Math.max(26, Math.round(R*1.0), Math.round(cvs.height*0.045)); strokeColoredText(p.text, p.x, p.y, `bold ${fontSize}px system-ui`, '#39ff14'); }else if(p.type==='sp'){ let fontSize = Math.max(54, Math.round(R*2.1), Math.round(cvs.height*0.11)); strokeOrangeWhiteText(p.text, p.x, p.y, `bold ${fontSize}px system-ui`); } ctx.globalAlpha=1; } }
function drawUI(){ if(gameState!=="playing") return; const cx=cvs.width - Math.max(70, Math.round(R*2.7)); ctx.textAlign='center'; ctx.fillStyle='#fff'; ctx.font=`bold ${Math.max(38,Math.round(R*1.7))}px system-ui`; ctx.fillText(`${combo}`, cx, Math.max(40,Math.round(R*1.4))); ctx.font=`bold ${Math.max(20,Math.round(R*0.9))}px system-ui`; ctx.fillText('COMBO', cx, Math.max(70,Math.round(R*2.0))); ctx.textAlign='right'; ctx.font=`bold ${Math.max(16,Math.round(R*0.7))}px system-ui`; ctx.fillText(`SCORE: ${score}`, cvs.width-12, cvs.height-16); }
function drawOverlays(){ const f=popups.find(p=>p.type==='flash'); if(f){ ctx.fillStyle=`rgba(255,255,255,${f.timer/f.duration*0.6})`; ctx.fillRect(0,0,cvs.width,cvs.height); } }
function drawSPCountdown(){ if(spCountdownTimer<=0) return; if(spCountdownValue===0 && Math.floor(frame/8)%2===0) return; const x=cvs.width/2, y=Math.round(cvs.height*0.25); ctx.textAlign='center'; ctx.font=`bold ${Math.max(48,Math.round(R*2.0))}px system-ui`; const g=ctx.createLinearGradient(x-80,y,x+80,y); const hue=(frame*4)%360; for(let i=0;i<=6;i++) g.addColorStop(i/6, `hsl(${(hue+i*60)%360},100%,50%)`); ctx.lineWidth=8; ctx.strokeStyle=g; ctx.strokeText(spCountdownValue, x, y); ctx.fillStyle='rgba(255,255,255,0.9)'; ctx.fillText(spCountdownValue, x, y); }
function drawACFailFlash(){ if(acFailFlashTimer > 0){ const alpha = Math.min(0.38, acFailFlashTimer/18*0.38); ctx.save(); ctx.globalAlpha = alpha; ctx.fillStyle = "#ff0033"; ctx.fillRect(0,0,cvs.width,cvs.height); ctx.globalAlpha = 1; ctx.restore(); } }
function drawSPBoostFrame() { if (typeof spBoostTimer === 'undefined' || spBoostTimer <= 0) return; const w = cvs.width, h = cvs.height; const maxT = 240; const fade = Math.max(0, Math.min(1, spBoostTimer / maxT)); const noteCount = 16; const colors = [ "#ff4c4c", "#ffa500", "#ffe93a", "#4cff4c", "#39bfff", "#8b4cff", "#ff6cff", ]; for (let i = 0; i < noteCount; i++) { const gridX = ((i % 4) + 0.5) / 4 * w; const gridY = ((Math.floor(i / 4) + 0.7) / 4) * h * 0.85 + h * 0.06; const t = spBoostTimer; const stepPeriod = 28 + (i % 5) * 7; const step = Math.floor((t + i * 13) / stepPeriod) % 4; const stepTable = [ {dx: 0, dy: 0, angle: 0}, {dx: 5, dy: -4, angle: 0.13}, {dx: -4, dy: 5, angle: -0.12}, {dx: 3, dy: -2, angle: 0.09} ]; const {dx, dy, angle} = stepTable[step]; const size = 20 + Math.sin(i * 2.3) * 3; ctx.save(); ctx.globalAlpha = 0.19 * fade; ctx.translate(gridX + dx, gridY + dy); ctx.rotate(angle); ctx.beginPath(); ctx.ellipse(0, 0, size * 0.38, size * 0.26, 0, 0, Math.PI * 2); ctx.fillStyle = colors[i % colors.length]; ctx.shadowColor = "#fff"; ctx.shadowBlur = 9; ctx.fill(); ctx.beginPath(); ctx.moveTo(size * 0.13, -size * 0.05); ctx.lineTo(size * 0.13, -size * 0.56); ctx.lineWidth = size * 0.10; ctx.strokeStyle = colors[i % colors.length]; ctx.lineCap = "round"; ctx.shadowBlur = 0; ctx.stroke(); ctx.beginPath(); ctx.moveTo(size * 0.13, -size * 0.56); ctx.quadraticCurveTo(size * 0.40, -size * 0.63, size * 0.33, -size * 0.28); ctx.lineWidth = size * 0.08; ctx.strokeStyle = colors[i % colors.length]; ctx.stroke(); ctx.restore(); } } 
...
function render(){
  ...
  if(gameState==="result"){
    ...
    const scoreFontSize = Math.round(cvs.height*0.07); ctx.font = `bold ${scoreFontSize}px system-ui`; ctx.lineWidth = 8; ctx.strokeStyle = "#000"; ctx.strokeText("ユメ語るよりユメ歌おう", 0, -36); ctx.fillStyle = "#fff"; ctx.fillText("ユメ語るよりユメ歌おう", 0, -36);
